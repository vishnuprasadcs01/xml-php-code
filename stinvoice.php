...
class ZatcaInvoiceBuilder
{
    ...
    /**
     * Placeholder for the ext:UBLExtensions signature block.
     * Real usage: sign the canonicalized invoice XML (minus this block and
     * the QR AdditionalDocumentReference) with your ECDSA P-256 private
     * key, embed the X.509 cert issued by ZATCA's CA, and populate the
     * digest/signature values here before this element is inserted.
     */
    public function addUblExtensionsPlaceholder(array $signature): void
    {
        ...
        $uri = $this->doc->createElementNS(self::NS_EXT, 'ext:ExtensionURI',
            'urn:oasis:names:specification:ubl:dsig:enveloped:xades');
        $content = $this->doc->createElementNS(self::NS_EXT, 'ext:ExtensionContent');

        // In production this ext:ExtensionContent holds the full
        // sig:UBLDocumentSignatures / ds:Signature / xades:QualifyingProperties
        // tree produced by your signing routine. Here we just note that it's
        // pending, so the structural placement is correct and easy to fill in.
        $comment = $this->doc->createComment(
            ' SIGNATURE PLACEHOLDER: insert sig:UBLDocumentSignatures ' .
            '(ds:Signature + xades:QualifyingProperties) produced by your ' .
            'ZATCA signing routine here. digest=' . ($signature['digest'] ?? 'PENDING')
        );

        $content->appendChild($comment);
        $extension->appendChild($uri);
    ...
    /**
     * PHP's DOMDocument re-declares xmlns:cac / xmlns:cbc / xmlns:ext on
     * every element created via createElementNS, even though they're
     * already declared on the root. This is harmless (still valid XML)
     * but bloats the output — this strips the redundant duplicates,
     * keeping only the root-level declarations.
     */
    private function stripRedundantNamespaceDeclarations(): void
    {
        $redundant = ['xmlns:cac', 'xmlns:cbc', 'xmlns:ext'];
...
$invoice->addUblExtensionsPlaceholder(['digest' => null]); // fill in via real signing step
...
// echo $invoice->toXml();
$invoice->save(__DIR__ . '/standard_invoice_vishnu.xml');